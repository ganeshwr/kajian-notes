...
import { Search, BookOpen, X, Copy, Plus, Loader } from 'lucide-react';
...
export const QuranSearch: React.FC<QuranSearchProps> = ({
  isOpen,
  onClose,
  onInsert
}) => {
  ...
  const handleSearch = async () => {
    ...
    setError('');
    try {
      const results = await QuranService.searchVerses(searchQuery);
      setSearchResults(results);
      if (results.length === 0 && searchQuery.trim()) {
        setError('No verses found. Try searching with references like "2:255" or keywords like "kursi", "fatiha"');
      }
    } catch (error) {
      console.error('Error searching Quran:', error);
      setError('Error searching verses. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleInsertVerse = (verse: QuranVerse) => {
    const formattedVerse = QuranService.formatVerseForInsertion(verse);
  ...
  const popularSearches = [
    ...
    { label: 'Al-Fatiha', query: '1:1' },
    { label: 'Al-Ikhlas', query: '112:1' },
    { label: 'Light Verse', query: '24:35' },
    { label: 'Ar-Rahman', query: '55:1' }
  ];
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white flex items-center">
              <BookOpen className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-islamic-600" />
              <span className="hidden sm:inline">Search Quran Verses</span>
              <span className="sm:hidden">Quran Search</span>
            </h2>
            <button
              onClick={onClose}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by reference (2:255) or keywords..."
              value={searchQuery}
          ...
          {/* Popular Searches */}
          <div className="mt-4">
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-2">Popular searches:</p>
          ...
          {loading ? (
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-islamic-700 dark:text-islamic-300 text-sm sm:text-base">
                        {QuranService.formatVerseReference(verse)}
                      </h3>
                    ...
                    <div className="flex space-x-1 sm:space-x-2 ml-2">
                      <button
                        onClick={() => copyToClipboard(`${QuranService.formatVerseReference(verse)}\n\n${verse.text}\n\n"${verse.translation}"`)}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-2">
                Try searching with a reference like "2:255" or keywords like "kursi", "fatiha"
              </p>
            </div>
          ) : (
            <div className="text-center py-8">
              <BookOpen className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base">
                Enter a search query to find Quran verses
              </p>
              <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-2">
                Search by reference (2:255) or try popular searches above
              </p>
            </div>
          )}
        </div>
